...
class MediatedAttention(Attention):
    ...
    def forward(self, x):
        ...
        # TO DO: add DVC, understand bias terms and check reshaping
        sq_q = q.transpose(-2, -1).reshape(B, C, latent_side, latent_side)
        t = self.mediate(sq_q).reshape(B, self.num_heads, self.head_dim, self.mediator_dim).transpose(-2, -1)
