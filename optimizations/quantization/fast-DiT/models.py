...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        profile = block_kwargs.pop("profile", False)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, profile=self.profile) for _ in range(depth)
        ])
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        combined = combined.to(self.x_embedder.proj.weight.dtype)
        model_out = self.forward(combined, t, y)
